...
genericDecodeJson :: forall a. (Generic a) => Options -> Json -> Either DecodeError a
genericDecodeJson opts json = maybe (Left $ decodeError Nil "fromSpine failed") Right <<< fromSpine
                =<< genericUserDecodeJson' opts Nil (toSignature (Proxy :: Proxy a)) json


-- | Generically decode json, using a supplied userEncoding, falling back to genericEncodeJson':
genericUserDecodeJson' :: Options -> History -> GenericSignature -> Json -> Either DecodeError GenericSpine
genericUserDecodeJson' opts'@(Options opts) history sign json = fromMaybe (genericDecodeJson' opts' history sign json)
                                                        (opts.userDecoding opts' history sign json)

-- | Decode `Json` representation of a `GenericSpine`.
genericDecodeJson' :: Options -> History -> GenericSignature -> Json -> Either DecodeError GenericSpine
genericDecodeJson' opts history signature json = case signature of
 ...
 SigArray thunk -> do
   ...
   SArray <$> traverse (map const <<< genericUserDecodeJson' opts history (thunk unit)) jArr
 SigRecord props -> do
   jObj <- mFail' "Expected an object" $ toObject json
   SRecord <$> for props \({recLabel: lbl, recValue: val}) -> do
     pf <- mFail (lbl : history) ("'" <> lbl <> "' property missing") (M.lookup lbl jObj)
     sp <- genericUserDecodeJson' opts (lbl: history) (val unit) pf
     pure { recLabel: lbl, recValue: const sp }
 SigProd typeConstr constrSigns -> genericDecodeProdJson' opts history typeConstr constrSigns json
 where
   mFail' :: forall a. String -> Maybe a -> Either DecodeError a
   mFail' = mFail history

genericDecodeProdJson' :: Options -> History -> String -> Array DataConstructor -> Json -> Either DecodeError GenericSpine
genericDecodeProdJson' opts'@(Options opts) history tname constrSigns json = unsafePartial $
  ...
  where
    ...
    decodeConstructor constr jVals = do
      ...
      sps <- zipWithA (\k -> genericUserDecodeJson' opts' history (k unit)) constr.sigValues vals
    ...
    findConstrFail tag = mFail history (decodingErr ("'" <> tag <> "' isn't a valid constructor")) (findConstr tag)
    findConstr tag = find ((tag == _) <<< fixConstr <<< _.sigConstructor) constrSigns



mFail :: forall a. History -> String -> Maybe a -> Either DecodeError a
mFail history msg = maybe (Left $ decodeError history msg ) (\a -> Right a)
